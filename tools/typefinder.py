...
def build_pipeline(args: argparse.Namespace) -> Pipeline:
    ollama_client = None if args.no_ollama else OllamaClient(
    ...
    return Pipeline(
        [
            DiscreteDataSource(args.uri, prebuffer_bytes=args.prebuffer_bytes, ollama_client=ollama_client),
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Determine file type using header or Ollama fallback.")
    ...
    parser.add_argument("--ollama-timeout", type=int, default=10, help="HTTP timeout for Ollama requests.")
