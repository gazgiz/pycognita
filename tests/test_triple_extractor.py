...
def test_iri_generation_priority():
    ...
    extractor._pads = [
        pad_hash
    ]  # Only looking at own pads? The code looks at `getattr(pad, "caps")` passed to on_buffer
...
def test_structured_list_prompt():
    ...
    extractor.on_buffer(
        sink_pad, "This input needs to be long enough to trigger the extractor logic." * 2
    )
