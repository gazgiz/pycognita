...
from cognita.source import DiscreteDataSource, TimeSeriesDataSource, TypeFinderError
...
def test_timeseries_source_header_detect(mock_pad):
    ...
    with (
        patch("builtins.open", mock_open(read_data=b"log entry")),
...
def test_timeseries_source_ollama_fallback(mock_pad):
    ...
    with (
        patch("builtins.open", mock_open(read_data=b"stream data")),
        patch("os.path.isfile", return_value=True),
    ):
        source.process()
...
def test_timeseries_source_fail_no_ollama(mock_pad):
    ...
    with (
        ...
        patch("os.path.isfile", return_value=True),
        pytest.raises(TypeFinderError, match="Ollama fallback not configured"),
    ):
        source.process()
...
def test_discrete_data_source(mock_pad):
    ...
    with (
        patch("builtins.open", mock_open(read_data=b"sample data")),
        patch("os.path.isfile", return_value=True),
    ):
        source.process()
